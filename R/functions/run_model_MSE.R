...
# for(i in 1:length(parms)) { assign(parms[i],parms.list[[i]]) }
# set.seed(seednum)
# harvmgmt="smsy_dlm_goal"

########################################## generate stock-recruit parameters
##==========================================## Ricker parameters and regimes
##------------------------------------------------------------------## alpha
alpha.low<-rlnorm(1,mean=log(alpha_mean),sd=sr_parms_sd) ## low production
...
beta<-rlnorm(1,mean=log(beta_corr),sd=sr_parms_sd)
} else { 
beta<-rlnorm(1,mean=log(beta_mean),sd=sr_parms_sd)
...
########################################## arrays for storing simualted data
nage<-length(ages)
sexes<-c("M","F")
nsex<-length(sexes)
...
sr<-ricker(spawn=PopDat$Esc[y],sig=procerr,alpha=alpha,beta=beta,rho=rho,last.eps=ifelse(y==1,0,PopDat$eps[y-1])) 
...
if(ricker_type=="const_beta")sr<-ricker(spawn=fecundity_total,sig=procerr,alpha=alpha_fec,beta=beta_fec,rho=rho,last.eps=ifelse(y==1,0,PopDat$eps[y-1]))
if(ricker_type=="const_rmax")sr<-ricker(spawn=fecundity_total,sig=procerr,alpha=alpha_fec,rmax=rmax_fec,rho=rho,last.eps=ifelse(y==1,0,PopDat$eps[y-1]))
...
if(ricker_type=="const_beta")sr<-ricker(spawn=eggmass_total,sig=procerr,alpha=alpha_egg,beta=beta_egg,rho=rho,last.eps=ifelse(y==1,0,PopDat$eps[y-1]))
if(ricker_type=="const_rmax")sr<-ricker(spawn=eggmass_total,sig=procerr,alpha=alpha_egg,rmax=rmax_egg,rho=rho,last.eps=ifelse(y==1,0,PopDat$eps[y-1]))
