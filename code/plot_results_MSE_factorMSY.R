...
# rm(list=ls()) 
##===============================================================## packages
pkgs<-c("here","tidyverse","dplyr","scales","RColorBrewer","readxl","fishualize","gsl","vioplot", "caroline","ggplot2","viridis","forcats","gridExtra","lemon","fmsb","MetBrewer","ggradar","showtext","DataCombine", "ggridges","ggExtra")
...
# if(futureT=="yes") { ny=ny } else { ny<-nyh }
##==============================================================## scenarios
...
##---------------------------------------------------------## scenario names
if(length(unique(scenarios$selectivity))!=1){ scenario_names<-paste0(scenarios$trends,"\n",scenarios$selectivity) }
if(length(unique(scenarios$mgmt))!=1){ scenario_names<-paste0(scenarios$trends,"\n",scenarios$mgmt) }
if(length(unique(scenarios$selectivity))!=1){ if(length(unique(scenarios$mgmt))!=1){ scenario_names<-paste0(scenarios$trends,"\n",scenarios$mgmt,"\n",scenarios$selectivity) } }
##--------------------------------------------------------## names of trends
...
plot_egg_mass$trends<-as.factor(as.character(plot_egg_mass$trends))
plot_egg_mass<-df_egg_mass %>% pivot_longer(!c(trends,factorMSY,mgmt), names_to="iteration", values_to="value") %>% data.frame()
plot_egg_mass<-dplyr::select(plot_egg_mass,-iteration,-mgmt)
plot_egg_mass$factorMSY<-as.factor(plot_egg_mass$factorMSY)
##-----------------------------------------------------------------## ggplot
jig<-position_dodge(width=0.6)
p<-plot_egg_mass %>% ggplot(aes(x=fct_inorder(trends),y=value,fill=fct_inorder(factorMSY)))+
  geom_violin(position=jig,trim=F,lwd=0.1,scale="width",width=0.6)+ 
  #geom_hline(yintercept=-49,size=0.1)+ ## Staton et al. 2021
  #geom_hline(yintercept=-28,size=0.1)+ ## Ohlberger et al. 2020
...
ggsave(paste0("percent_change_eggmass_",nyrs,"yrs.pdf"),p,width=4.2,height=5.5,units="in")
...
ggsave("S_msy_by_method_quantiles.pdf",g,width=5,height=4,units="in")  
...
ggsave("S_msy_by_method_medians.pdf",g,width=5,height=4,units="in")  
...
g<-p+facet_grid(cols=vars(factorMSY),scale="free")+theme(strip.text.x=element_text(size=10),strip.text.y=element_text(size=10),legend.position=c(0.1,0.8)) 
ggsave("S_msy_by_method_probability.pdf",g,width=5,height=4,units="in")  
...
trend_names<-c("no trends","age-sex trends","ASL trends stabilized","ASL trends continued" )
...
df_pl$MSYfactor<-factor(df_pl$factorMSY,levels=c(0.75,1.5),labels=c("aggressive","conservative"))
...
ggsave("Future_prob_above_threshold.pdf",p,width=4.5,height=3,units="in")
...
df_av_harv<-dplyr::filter(df_av_harv,method!="None")
##----------------------------------------------------------## select trends
...
df_av_ret<-dplyr::filter(df_av_ret,method!="None")
##----------------------------------------------------------## select trends
...
ggsave("Future_mean_return_differences.pdf",g,width=6,height=2.5,units="in")
...
ggsave("Future_mean_harvest_escapement_differences.pdf",g,width=6,height=4,units="in")
...
##----------------------------------------------------------## select trends
df_risk_plot<-dplyr::filter(df_risk_plot,trends!="age-sex trends")
...
# ggsave("Metrics_prob_above_threshold.pdf",p,width=4.5,height=3,units="in")
...
ggsave("Metrics_prob_above_threshold.pdf",g,width=3,height=6,units="in")
...
##----------------------------------------------------------## select trends
df_diff_plot<-dplyr::filter(df_diff_plot,trends!="age-sex trends")
...
ggsave(paste0("Performance-difference-and-density-",use_factorMSY,".pdf"), g,width=4,height=4, units="in")
...
##----------------------------------------------------## 'method'
df$method<-"None"
df$method[grepl("eq",df$mgmt)]<-"Explicit (YPR)"
df$method[grepl("dlm",df$mgmt)]<-"Implicit (DLM)"
##----------------------------------------------------## median all metrics
...
g<-p+facet_grid(rows=vars(metric_label),cols=vars(factorMSY),scale="free_y")+theme(strip.text.x=element_text(size=10),strip.text.y=element_text(size=10))
...
##----------------------------------------------------------## metric labels
labs<-c("Mean harvest","Harvest stability","Mean return","P > threshold")
n_metrics<-length(labs)
##==========================================================## long format
...
##----------------------------------------------------------## select trends
dfp<-dplyr::filter(dfp,trends!="age-sex trends")
...
ggsave(paste0("Performance_metrics.pdf"),g,width=4.2,height=6,units="in")
...
g<-p+facet_grid(rows=vars(metric_label),cols=vars(factorMSY),scale="free_y")+theme(strip.text.x=element_text(size=9),strip.text.y=element_text(size=9))
ggsave(paste0("Performance_metric_few.pdf"),g,width=4,height=4,units="in")
...
for(j in 1:n_used_fac) {
  ...
  for(i in 1:n_trend_used) {
    ...
    if(scale=="across") {
      df_gg<-data.frame(df_use)
      df_gg$Mean.harvest<-(df_gg$Mean.harvest-mins$min[mins$metric_label=="Mean harvest"])/(maxs$max[maxs$metric_label=="Mean harvest"]-mins$min[mins$metric_label=="Mean harvest"])
      df_gg$Harvest.stability<-(df_gg$Harvest.stability-mins$min[mins$metric_label=="Harvest stability"])/(maxs$max[maxs$metric_label=="Harvest stability"]-mins$min[mins$metric_label=="Harvest stability"])
      df_gg$P...threshold<-(df_gg$P...threshold-mins$min[mins$metric_label=="P > threshold"])/(maxs$max[maxs$metric_label=="P > threshold"]-mins$min[mins$metric_label=="P > threshold"])
      df_gg$Mean.return<-(df_gg$Mean.return-mins$min[mins$metric_label=="Mean return"])/(maxs$max[maxs$metric_label=="Mean return"]-mins$min[mins$metric_label=="Mean return"])
    }
    ##------------------------------------------------## make ggradar plot
...
ggsave("Radarplots.pdf",plts_all,width=4*n_used_fac,height=3*n_trend_used,units="in")
